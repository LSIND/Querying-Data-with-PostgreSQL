...
-- данные из CONCAT(companyname, ': ', contactname) не помещаются в столбец fullname размера 25
--

-- Вариант решения 1:
-- Обрезать данные CONCAT(companyname, ': ', contactname) до 25 символов типа вставке данных
-- (только в лабораторной среде!)

INSERT INTO public.extracustomers(fullname, city, country, discount )
SELECT CAST(CONCAT(companyname, ': ', contactname) as varchar(25)) AS  fullname, city, country, 0.4
FROM sales.customers
WHERE country IN ('Germany', 'France');


-- Вариант решения 2:
-- Изменение размера столбца fullname в таблице (операция DDL) до 100 символов
--
...
----------------------
--
-- Вариант решения 3 (подойдет для лабораторной среды):
-- Удалить таблицу
...
-- Task 7
-- 
-- Скопируйте данные о продуктах дороже 50 у.е. из таблицы production.products в несуществующую таблицу public.newproducts.
...
WHERE e1.custid > e2.custid AND e2.city = e1.city and e1.country = e2.country;
