...
-- THIS WILL FAIL
SELECT -- попытка написать запрос "классическим" подходом
	...
	(
		...
		WHERE EXTRACT(YEAR FROM o2.orderdate) <= EXTRACT(YEAR FROM o.orderdate)
	) AS runsales
FROM "Sales"."Orders" AS o
INNER JOIN "Sales"."OrderDetails" AS d 
ON d.orderid = o.orderid
GROUP BY EXTRACT(YEAR FROM o.orderdate)
ORDER BY orderyear; -- ошибка в GROUP BY
